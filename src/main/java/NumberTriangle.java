...
public class NumberTriangle {
    ...
    public int retrieve(String path) {
        ...
        for (int idx = 0; idx < path.length(); idx++) {
            char way = path.charAt(idx);
            if (way == 'l') {
                if (current.left == null) {
                    throw new IllegalArgumentException("Path goes past leaf at index " + idx);
                }
                ...
                throw new IllegalArgumentException("Invalid path character: " + way);
            }
        }
        return current.getRoot();
    }
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        List<NumberTriangle> prevList = new ArrayList<>();
        ...
        while (line != null) {
            ...
            for (int i = 0; i < parts.length; i++) {
                currentList.add(new NumberTriangle(Integer.parseInt(parts[i])));
            }
            if (prevList.isEmpty()) {
                top = currentList.get(0);
            }
            else{
